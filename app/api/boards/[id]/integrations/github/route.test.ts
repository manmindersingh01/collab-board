import { describe, it, expect, vi, beforeEach } from "vitest";
import { mockRequest, parseResponse } from "@/tests/setup";
import prisma from "@/lib/prisma";
import { getDbUser } from "@/lib/user";
import { POST, DELETE } from "./route";

const mockUser = {
  id: "user1",
  name: "Test",
  email: "test@test.com",
  clerkId: "clerk1",
  avatarUrl: null,
  createdAt: new Date(),
  updatedAt: new Date(),
};

beforeEach(() => {
  vi.clearAllMocks();
});

describe("POST /api/boards/[id]/integrations/github", () => {
  const params = Promise.resolve({ id: "board1" });

  it("returns 401 when not authenticated", async () => {
    vi.mocked(getDbUser).mockResolvedValue(undefined);
    const res = await POST(
      mockRequest("POST", { repo: "org/repo", token: "ghp_xxx" }),
      { params },
    );
    expect(res.status).toBe(401);
  });

  it("returns 403 when user is not owner", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "editer",
      joinedAt: new Date(),
    });
    const res = await POST(
      mockRequest("POST", { repo: "org/repo", token: "ghp_xxx" }),
      { params },
    );
    expect(res.status).toBe(403);
  });

  it("returns 400 for invalid repo format", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    const res = await POST(
      mockRequest("POST", { repo: "invalid-repo", token: "ghp_xxx" }),
      { params },
    );
    expect(res.status).toBe(400);
  });

  it("returns 400 for missing repo", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    const res = await POST(
      mockRequest("POST", { token: "ghp_xxx" }),
      { params },
    );
    expect(res.status).toBe(400);
  });

  it("returns 400 for missing token", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    const res = await POST(
      mockRequest("POST", { repo: "org/repo" }),
      { params },
    );
    expect(res.status).toBe(400);
  });

  it("creates GitHub integration on success", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    vi.mocked(prisma.integration.upsert).mockResolvedValue({
      id: "int1",
      type: "github",
      config: { repo: "org/repo", token: "ghp_xxx", webhookSecret: "uuid" },
      isActive: true,
      boardId: "board1",
      createdAt: new Date(),
      updatedAt: new Date(),
    });

    const res = await POST(
      mockRequest("POST", { repo: "org/repo", token: "ghp_xxx" }),
      { params },
    );
    const { status, body } = await parseResponse(res);

    expect(status).toBe(201);
    expect(body.type).toBe("github");
    expect(body.webhookSecret).toBeDefined();
  });
});

describe("DELETE /api/boards/[id]/integrations/github", () => {
  const params = Promise.resolve({ id: "board1" });

  it("returns 401 when not authenticated", async () => {
    vi.mocked(getDbUser).mockResolvedValue(undefined);
    const res = await DELETE(mockRequest("DELETE"), { params });
    expect(res.status).toBe(401);
  });

  it("returns 403 when user is not owner", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "viewer",
      joinedAt: new Date(),
    });
    const res = await DELETE(mockRequest("DELETE"), { params });
    expect(res.status).toBe(403);
  });

  it("returns 404 when no github integration", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    vi.mocked(prisma.integration.findUnique).mockResolvedValue(null);

    const res = await DELETE(mockRequest("DELETE"), { params });
    expect(res.status).toBe(404);
  });

  it("deletes github integration on success", async () => {
    vi.mocked(getDbUser).mockResolvedValue(mockUser);
    vi.mocked(prisma.boardMember.findUnique).mockResolvedValue({
      boardId: "board1",
      userId: "user1",
      role: "owner",
      joinedAt: new Date(),
    });
    vi.mocked(prisma.integration.findUnique).mockResolvedValue({
      id: "int1",
      type: "github",
      config: {},
      isActive: true,
      boardId: "board1",
      createdAt: new Date(),
      updatedAt: new Date(),
    });
    vi.mocked(prisma.integration.delete).mockResolvedValue({
      id: "int1",
      type: "github",
      config: {},
      isActive: true,
      boardId: "board1",
      createdAt: new Date(),
      updatedAt: new Date(),
    });

    const res = await DELETE(mockRequest("DELETE"), { params });
    const { status, body } = await parseResponse(res);

    expect(status).toBe(200);
    expect(body.success).toBe(true);
  });
});
